import { ActionIcon, type ActionIconProps, Group, Kbd, Text, Tooltip, type TooltipProps } from "@mantine/core";
import { formatForDisplay, type Hotkey, useHotkey } from "@tanstack/react-hotkeys"
...
export function IconButton({ icon, onClick, hotkey, description, ...actionIconProps }: Props) {
  ...
  const tooltipProps = useMemo((): TooltipProps => {
    if (description && hotkey) {
      console.log(description, hotkey)
      return {
        label: (
          <Group>
    ...
    if (!description && hotkey) return { label: formatForDisplay(hotkey) };
